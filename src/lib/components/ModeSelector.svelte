<script lang="ts">
	...
	function handleClickOutside(e: MouseEvent) {
		...
		if (charDropdownEl && !charDropdownEl.contains(e.target as Node)) {
			charOpen = false;
		}
	}

	$effect(() => {
		if (modeOpen || charOpen) {
			document.addEventListener('click', handleClickOutside, true);
			return () => document.removeEventListener('click', handleClickOutside, true);
		}
	});

	let charSummaryText = $derived.by(() => {
		const parts: string[] = [];
		for (const r of rows) {
...
<div class="flex flex-wrap gap-3">
	<div class="relative min-w-[140px] flex-1" bind:this={modeDropdownEl}>
		<span class="dark:text-gray-300 mb-1 block text-sm font-medium text-gray-700">Mode</span>
		<button
			type="button"
		...
		>
			...
			<svg
				class="ml-2 h-4 w-4 shrink-0 text-gray-400 transition-transform {modeOpen ? 'rotate-180' : ''}"
				fill="none"
				viewBox="0 0 24 24"
				stroke="currentColor"
				stroke-width="2"
		...
		{#if modeOpen}
			<div class="dark:border-gray-600 dark:bg-gray-800 absolute z-10 mt-1 w-full overflow-y-auto rounded-lg border border-gray-200 bg-white py-1 shadow-lg">
				{#each Object.entries(MODE_LABELS) as [key, label]}
					<button
						type="button"
						onclick={() => pickMode(key as QuizMode)}
	...
	{#if mode !== 'combined'}
		<div class="relative min-w-[200px] flex-1" bind:this={charDropdownEl}>
			<span class="dark:text-gray-300 mb-1 block text-sm font-medium text-gray-700">Characters</span>
			<button
				type="button"
				aria-haspopup="listbox"
				aria-expanded={charOpen}
				onclick={() => (charOpen = !charOpen)}
				class="dark:border-gray-600 dark:bg-gray-700 dark:text-white flex w-full items-center justify-between rounded-lg border border-gray-300 bg-white px-3 py-2 text-left text-sm shadow-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none"
			>
				<span class="truncate">{charSummaryText}</span>
				<svg
					class="ml-2 h-4 w-4 shrink-0 text-gray-400 transition-transform {charOpen ? 'rotate-180' : ''}"
					fill="none"
					viewBox="0 0 24 24"
					stroke="currentColor"
					stroke-width="2"
